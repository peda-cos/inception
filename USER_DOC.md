...
- **URL**: `https://portainer.peda-cos.42.fr`
...
```bash
cat /home/peda-cos/secrets/credentials.txt   # WordPress admin and user passwords
...
| WordPress user | `editor` | `secrets/credentials.txt` → `WORDPRESS_USER_PASSWORD=...` |
...
Another service on your machine may be using port 443 or 21. Stop the conflicting service or check with:

```bash
sudo ss -tlnp | grep -E '443|:21'
```
